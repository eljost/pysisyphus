...
def update(frame, *fargs):
    ...
    imagex = coords[frame][:,0]
    imagey = coords[frame][:,1]
    p = ax.plot(imagex, imagey, "ro", ls="-")
    return p, 


def anim(optimizer):
    fig, ax = plt.subplots()
    cycles = range(optimizer.cur_cycle)
    #uo = lambda c: update(ax, optimizer, c)
    #an = animation.FuncAnimation(fig, uo, cycles, interval=500)#, blit=True)
    an = animation.FuncAnimation(fig, update, cycles, interval=500, fargs=(ax, optimizer))#, blit=True)
