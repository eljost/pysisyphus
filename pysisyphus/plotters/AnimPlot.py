...
class AnimPlot:
    def __init__(
        ...
        if not self.growing:
            # Total forces
            self.total_forces_quiv = self.ax.quiver(
                images_x, images_y, forces_x, forces_y
            )
            # Tangents
            self.tangent_quiv = self.ax.quiver(
                images_x, images_y, tangents_x, tangents_y, color="b"
            )
    ...
    def func(self, frame):
        ...
        if not self.growing:
            ...
            offsets = np.stack((images_x, images_y), axis=-1)
