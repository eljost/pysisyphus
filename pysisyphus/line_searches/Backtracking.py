...
class Backtracking(LineSearch):
    ...
    def alpha_new_from_phi(self, cycle, phi0, dphi0, alpha, alpha_prev):
        ...
        self.log(f"\tCycle {cycle:02d}: alpha={alpha:.6f}, ϕ={phi_i:.6f}")
    ...
    def alpha_new_from_phi_dphi(self, cycle, phi0, dphi0, alpha):
        ...
        # Constrained quartic interpolation
        res = quartic_fit(phi0, phi_i, dphi0, dphi_i)
        if res:
            alpha_new = res.x * alpha
        self.log(f"\tNew alpha from quartic interpolation: {alpha_new:.6f}")
        return alpha_new
    ...
    def run_line_search(self):
        ...
        for i in range(self.max_cycles):
            if self.use_grad:
                alpha_new = self.alpha_new_from_phi_dphi(i, phi0, dphi0, alpha)
            else:
                alpha_new = self.alpha_new_from_phi(i, phi0, dphi0, alpha, alpha_prev)
